    ...
    detailArray.forEach( currentDetailValue => {
        ...
        if ( ToCPageNos
          && !writePageNoToArray ) {
            ...
            if ( print )
                text += `<td style="width: 80%;">
                            <p class="contentLayer"><b>${chapter}.${subChapterNo} ${title}</b></p></td>
                        <td style="width: 15%; text-align: right">${pageNo}`;
                //text += `<p class="contentLayer"><b>${chapter}.${subChapterNo} ${title}</b> ${pageNo}</p>`;
            else
                text += `<td style="width: 80%;">
                            <p class="contentLayer"><b>${chapter}.${subChapterNo} <a href="#${helper.GetID( chapter + '.' + subChapterNo )}">${title}</a></b></p></td>
                        <td style="width: 15%; text-align: right">${pageNo}`;
                //text += `<p class="contentLayer"><b>${chapter}.${subChapterNo} <a href="#${helper.GetID( chapter + '.' + subChapterNo )}">${title}</a></b> ${pageNo}</p>`;    
                
                //text += `<p class="contentLayer"><b>${chapter}.${subChapterNo} <a href="#${chapter}.${subChapterNo}">${title}</a></b> ${pageNo}</p>`;
                //text += `<p><a href="#${chapter}.${subChapterNo}">${chapter}.${subChapterNo} ${title}</a></p>`;
        }
        else {
            if ( print )
                text += `<td style="width: 95%;">
                            <p class="contentLayer"><b>${chapter}.${subChapterNo} ${title}</b></p>`;
            else
                text += `<td style="width: 95%;">
                            <p class="contentLayer"><b>${chapter}.${subChapterNo} <a href="#${helper.GetID( chapter + '.' + subChapterNo )}">${title}</a></b></p>`;
                //text += `<p class="contentLayer"><b>${chapter}.${subChapterNo} <a href="#${chapter}.${subChapterNo}">${title}</a></b></p>`;
                //text += `<p><a href="#${chapter}.${subChapterNo}">${chapter}.${subChapterNo} ${title}</a></p>`;
        }
            ...
            if ( currentDetail.type == 'HEADING' ) {
                ...
                if ( ToCPageNos
                  && !writePageNoToArray ) {
                    ...
                    if ( print )
                        text += `<p class="contentLayer"><b>${chapterNo}. ${title}</b></p></td>
                                    <td style="width: 15%; text-align: right">${pageNo}`;
                    else
                        text += `<p class="contentLayer"><b>${chapterNo}. <a href="#${chapterNo}">${title}</a></b></p></td>
                                    <td style="width: 15%; text-align: right">${pageNo}`;
                }
                else {
                    if ( print )
                        text += `<p class="contentLayer"><b>${chapterNo}. ${title}</b></p>`;
                    else
                        text += `<p class="contentLayer"><b>${chapterNo}. <a href="#${chapterNo}">${title}</a></b></p>`;
                }
                if ( writePageNoToArray )
...
const generateHTMLText = ( opinion , opinionDetails , hasToC , print , ToCPageNos , headingsArray ) => {
    return GetBody( opinion , opinionDetails , hasToC , print , ToCPageNos , headingsArray );
}
