const actionCodes = require('./actioncodes');
...
const renderTemplate = ( item , depth = 0 ) => {
    if ( item.type == 'HEADING'
      ...
      || item.type == 'TODOITEMACTIONHEAD' ) {
        ...
        if ( item.actionCode && actionCodes[item.actionCode] ) {
            renderedTemplate += template.replace( /\{\{actionCodeText\}\}/g, (item.actionCode && actionCodes[item.actionCode].text) || '' )
